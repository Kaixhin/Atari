...
You can use a custom environment (as the path to a Lua file) using `-env`, as long as the class returned respects the `rlenvs` [API](https://github.com/Kaixhin/rlenvs#api). One restriction is that the state must be represented as a single tensor (with arbitrary dimensionality). To prevent massive memory consumption, states are discretised to integers in [0, 255] in experience replay memory, assuming the state is comprised of reals in [0, 1] - this can be disabled with `-discretise false`. Visual environments can make use of explicit `-height`, `-width` and `-colorSpace` options to perform preprocessing for the network.
